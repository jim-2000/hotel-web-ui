  ...
  return (
    <div className=''>
        <nav className=' px-5 py-2 bg-white backdrop-blur-lg shadow-lg md:flex md:justify-between md:items-center'>
            ...
            `}>
                {
                    navlinks.map((item, index) => <NavlinkLIS key={index} index={index} item={item} router={router}  />)
                }              
...
function NavlinkLIS({item, index, router}) {
    return  <Link href={item.href} key={index}>
    <li key={index} className='mx-1 my-6 md:my-0 md:mx-2 hover:bg-slate-300 md:hover:bg-transparent '>
        <div className={`text-text text-sm mr-1 font-semibold transition-all ease-in   
        duration-0 hover:duration-500
         hover:text-main hover:font-bold 
         hover:text-base hover:tracking-widest
         focus:nav-active
         ${router.pathname == item.href ? 'nav-active' : '' }`}>{item.name}</div>
